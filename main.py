...
class PredictAndMapIdeal():
    def __init__(self,train_csv_path, ideal_csv_path, ):
        """
        ...
        mapping of ideal data based on the provided training data and ideal 
        data and test CSV files.It allows you to load the data from CSV files 
        and perform various operations for prediction and mapping.
        """
    ...
    def insert_train_ideal(self, train_table, ideal_table):
        ...
        This method takes two DataFrames, `train_df` and `ideal_df`, and inserts
        them into the corresponding database tables specified by `train_table_name`
        and `ideal_table_name` using the `df_to_sql` method of the 
    ...
    def predict_ideal_functions(self):
        ...
        try:
            ...
            # to remove naming conflicts with ideal y1, y2, y3, y4 columns 
            # with train y1, y2, y3, y4 columns
            # added suffixes to columns in train
            train_ideal_merged_df = pd.merge( self.train_df , self.ideal_df ,
                                              on ="x", suffixes=('_train',''))

            # Provide 'suffixes' as a tuple.
            # These suffixes are used to disambiguate columns with the same name
            #  in the original DataFrames train_df and ideal_df. 
            
            # When you merge DataFrames based on a common column 
            # (in your case, "x"), 
            # the common column used for the merge (i.e., "x") will not get
            #  the suffixes added
                ...
                if train_col not in columns_to_exclude:
                    ...
                    for ideal_col in self.ideal_df.columns:
                        if ideal_col not in columns_to_exclude:
                            # since suffixes used only for train df columns in
                            #  merged df,storing with same column names 
                            # y1...y50 in deviation_df
                            deviation_df[ideal_col] = (
                                train_ideal_merged_df[f'{train_col}_train']
                            - train_ideal_merged_df[ideal_col])

                    # here used square deviation & summed & later 
                    # we use absolute deviation in 2nd criteria
                    sum_values =  deviation_df.apply(lambda x: (x ** 2).sum())
                
                    # since have same no of rows or length we can use sum or 
                    # mean of deviations to predict ideal functions

                    lowest_sum_index = sum_values.idxmin() 
                    # finds the index, index==coloumn name

                    # storing the lowest sum column name 
                    # in selected_ideal_column_names
                    selected_ideal_column_names.append(lowest_sum_index)
                    

                    # storing the absolute deviation of (train - ideal_ to find 
                    # largest deviation for criteria 2 
                    selected_ideal_deviations_df[lowest_sum_index]= (
                    deviation_df[lowest_sum_index].abs() )

                    #creating individual deviation plots for each train_y 
                    # and selected ideal_y
                    plot=bokeh_train_ideal_plot(train_ideal_merged_df['x'],
                                    train_ideal_merged_df[f"{train_col}_train"], 
                                    train_ideal_merged_df[lowest_sum_index],
                                    label_1="train_{}".format(train_col), 
                                    label_2="ideal_{}".format(lowest_sum_index),)
                    # appending each deviation plot for each 
                    # selected ideal(4 selected)
                    plots.append(plot)

            # vizualizing each deviation between selected ideal & 
            # respective train as 2 plots in a row
            ...
            # in selected ideal cols names added x, 
            # we need x col for further usage
            selected_ideal_column_names.insert(0, 'x') 
            # based on selected ideal column names fetching df
            self.selected_ideal_df = (
                train_ideal_merged_df.loc[:, selected_ideal_column_names])
            # calculating max/largest deviation of selected ideal  cols
            max_devs_of_selected_ideal=selected_ideal_deviations_df.max()
            # returning selected ideal function dataframe & 
            # largest deviation of selected ideal cols
    ...
    def map_ideal_to_individual_test(self, x_test, y_test):
        ...
        '''
        # since x_test read as string by csv.reader, convert df[x] as string 
        # with as.type(str) just for comparing
        row = self.selected_ideal_df[self.selected_ideal_df['x'].astype(str) 
                                     == str(x_test)] 
        ...
        # to get just the value we use values method series.values[0]
        #  since series.values gives value in array
        ...
        no_of_ideal_function = (
          lowest_deviation_column 
          if lowest_deviation_value <= tolerance else 'pass')
        ...
        # after getting value it wil be in array 
        # eg: [4.536] so using index[0] to access value
        mapped_ideal_value = (
            row[no_of_ideal_function].values[0] 
            if no_of_ideal_function in row.columns else None)

        data_values_dict={"x_test":x_test, "y_test":y_test, "delta_y":delta_y, 
                          "no_of_ideal_function":no_of_ideal_function, 
                          "mapped_ideal":mapped_ideal_value}
        return data_values_dict
    # criteria 2
    # after traindata & ideal data loaded to db, now test data 
    # must be loaded line-by-line.Dont use pd.read converts to df, 
    # instead read line by line with csv module/package
    # assuming read line by line as read & insert in to db 
    # line by line (row by row from csv)
    def map_ideal_to_all_test_and_insert(self, test_csv_path, 
                                         test_mapping_table_name, 
                                         table_col_names):
        """
        ...
        # and also inserting each row in db as per requirement 
        # (but how ever we can save all in df & insert df in table at once)
        ...
        with open(test_csv_path, 'r', newline='\n') as csv_file:
            ...
            for row in csv_reader:
                ...
                data_values_tuple=(data_values_dict["x_test"], 
                                   data_values_dict["y_test"], 
                                   data_values_dict["delta_y"], 
                                   data_values_dict["no_of_ideal_function"]
                                   .replace("y", "N"))

                
                # inserting to db line by line, instead can insert bulk also
                # passing 2 arguments table name & data in dictionary format
                self.db_connection.insert_data(test_mapping_table_name, 
                                               table_col_names, data_values_tuple)
        bokeh_test_mapped_ideal(x_test_var, y_test_var, 
                                mapped_ideal, label_1="Test Data",
...
table_col_names=( "X (test func)", "Y (test func)", "Delta Y (test func)", "No. of ideal func",)
predictAndMapIdealInstance.map_ideal_to_all_test_and_insert(test_csv_path, 
                                                        test_mapping_table_name, 
                                                        table_col_names)
